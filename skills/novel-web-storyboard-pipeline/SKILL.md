...
Run a resumable local-app-and-web production workflow for at most two chapters per run. When the user asks for storyboard prompts or end-to-end production from novel text, first obtain the canonical cinematic prompt document in the **Doubao desktop app** through its installed `cinematic-storyboard-prompt-generator` skill, then derive the package, assets, and Seedance production from that accepted document. If the desktop-app control channel cannot bind after its bounded recovery, use the user's already signed-in **incognito Doubao website** fallback with `工作任务` → `云电脑` → `技能/cinematic-storyboard-prompt-generator`. Preserve the user's chosen surfaces: Doubao desktop app or this explicit browser fallback for director prompts, ChatGPT website for images, and Doubao website with Seedance 2.0 Fast for videos. Never substitute an API.
...
- Use the already signed-in Doubao **desktop app** for the director-prompt stage by default. Only when its control channel has failed its bounded recovery may the user-authorized signed-in incognito Doubao website be used for the same director task, with `工作任务` → `云电脑`. Use the connected external browser for the ChatGPT image stage and the Doubao Seedance video stage.
