...
Persist two separate account fields: `director_account` is the verified account that runs step 3; `video_account` is the account currently submitting Seedance jobs. Initialize `video_account = director_account` after the canonical prompt is accepted. The director request does not increment Seedance use. Increment a video account only on an official job acknowledgement, keep it for up to `account_generation_limit` acknowledged jobs in the current quota cycle, then advance the cursor without changing the accepted prompt.
...
During step 3 and before accepting `07`, compare every spoken line with the fixed 10-second group duration. Do not shorten, cut off, or replace approved dialogue. When ordinary delivery will overrun, require the web director output itself to specify the locked voice, complete line, and controlled accelerated delivery, normally 1.10–1.50x chosen for dialogue density, emotion, and intelligibility. Keep consonants and emotional intent intelligible. If even controlled acceleration cannot make the line intelligible, require the web director to split the dramatic beat into an additional storyboard group before acceptance. Never add or change this timing instruction locally after `07` is accepted.

Before accepting a multi-character SG in step 3, require the web director output to define stable screen placement before action: foreground/midground/background or left/center/right, each actor's movement lane, and the camera axis. Require staggered entries, attacks, landings, and reactions in time and depth, plus explicit prohibitions on body overlap, intersecting robes or weapons, shared landing positions, and actors passing through one another. Never insert these instructions into an already accepted SG locally.
...
For a director-prompt recovery that reaches a fresh controllable Doubao page, do not resume from the old page's account assumption. Resolve the target account from the current user request; if none is named, use `director_prompt.default_account_label`, currently `用户867998`. Read the newly rendered profile label and require an exact match before opening the skill menu. The target account must visibly offer `Cinematic Storyboard Prompt Generator`; a page showing another account or no target skill is an account mismatch, not a substitute route. Switch to the resolved account, verify the label and the actual skill, then continue. Never upload source references or submit the director task while either is mismatched or unknown.
...
- Account limit: after the initial director/video account reaches three officially acknowledged jobs or the page reports exhaustion, switch to another ordinary account; then use the reserved tail queue. Never rotate on an unknown or pending submission.
...
Log each rejection with shot ID, time range, evidence, defect class, and repair decision. Preserve the canonical original and every returned variant. Regenerate the complete shot with the exact accepted canonical SG body and the same required binding legend; do not create a locally rewritten interval prompt, bridge prompt, or altered full-shot prompt. Save each retry as `<chapter>-<shot>-重制-v<version>.mp4`, never overwrite or delete prior variants, and use the accepted retry's tail frame for any direct continuation that must also be regenerated.
