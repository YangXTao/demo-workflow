...
Use the visible `+` material button and the browser file-chooser flow first. Start `waitForEvent("filechooser")`, click that visible button, then call `chooser.setFiles()` once with all binding paths in their manifest order. Check `chooser.isMultiple()` first. This avoids duplicate thumbnails.

Do not click the hidden `input[type=file]` directly: it may not open a chooser. Do not use `locator.setInputFiles`; the supported surface is the file chooser. If a chooser upload fails because Chrome's extension cannot access local files, first verify that the ChatGPT extension has **Allow access to file URLs** enabled. If that is already enabled yet the chooser event itself remains unavailable, use the user-authorized Windows file-copy → Doubao paste fallback: copy only one validated binding at a time, paste it into the same unsubmitted composer, and verify the thumbnail count increases by exactly one before continuing. Preserve manifest order strictly; never paste a whole second batch after an unknown outcome. Do not open an uncontrolled native picker or request manual file selection.
